...
export function LanguageSwitch({ isOpen, selectedLanguage, onLanguageChange, onClose, anchorRef, mobileMode }: LanguageSwitchProps) {
  ...
  const baseClasses = mobileMode
    ? 'absolute left-3 right-3 top-full mt-3 flex flex-col gap-2 bg-black/95 border border-[var(--color-primary)]/20 px-3 py-3 rounded-xl shadow-[0_8px_24px_rgba(0,0,0,0.6)] backdrop-blur-[6px] animate-[slideDown_300ms_ease-out]'
