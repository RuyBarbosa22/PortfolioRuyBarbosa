...
export const HeroLeft: React.FC<HeroLeftProps> = ({
  onPrimaryClick,
  language = "pt",
}) => {
  ...
  const ctaDownload =
    language === "pt"
      ...
      : "Download CV";
  return (
    <div className="text-center lg:text-left flex flex-col xl:flex-1 w-full md:items-center lg:items-start xl:justify-center">
      <div className="w-full flex flex-col items-center lg:items-start mt-8 md:mt-0">
  <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-[1.4] pb-2 overflow-visible text-center lg:text-left sm:whitespace-nowrap">
        ...
        >
          ...
          >
            ...
            {language === "es" && (
              <span className="inline-block lg:whitespace-nowrap">
                Y soy un desarrollador
              </span>
            )}
            {language === "en" && (
              <span className="inline-block lg:whitespace-nowrap">I'm a</span>
            )}
            <TypewriterText
      ...
      <p className="text-sm sm:text-base md:text-lg text-gray-400 font-['Roboto_Mono',monospace] font-normal mb-10 md:mb-12 xl:mb-14 max-w-[90vw] sm:max-w-280px] xl:max-w-xl leading-relaxed lg:mx-0 text-justify lg:px-0">
        {language === "pt" && (
          <>
            Explore minhas paixões tecnológicas e projetos. Clique à vontade e
            descubra uma surpresa interativa que responde suas perguntas. Vamos
            criar algo incrível juntos!
          </>
        )}
        {language === "es" && (
          <>
            Explora mis pasiones tecnológicas y proyectos. Haz clic para
            descubrir una sorpresa interactiva que responde tus preguntas.
            ¡Vamos a crear algo increíble juntos!
          </>
        )}
        {language === "en" && (
          <>
            Explore my tech passions and projects. Click around to discover an
            interactive surprise that answers your questions. Let's build
